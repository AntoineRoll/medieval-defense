...
func _ready():
	print("=== WAVE 4 SIMULATION ===")
	
	# Spawn 4 units at random positions (like player would)
	var unit_scene = preload("res://scenes/foot_soldier.tscn")
	for i in range(4):
		var unit = unit_scene.instance()
		var angle = rand_range(0, 2*PI)
		unit.position = Vector2(640, 360) + Vector2(cos(angle), sin(angle)) * 200
		$".".add_child(unit)
		unit.setup_stats(100, 80, 120, 20, 10, 1.0)
	
	# Spawn 4 enemies at edges (wave 4)
	...
	print("4 units + 4 enemies spawned")
	print("Expected: Units kill enemies before they reach base")
